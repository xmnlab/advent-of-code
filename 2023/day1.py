...
calc_fuel_required = lambda mass: mass // 3 - 2
...
# test
for mass, result in test_set:
    print('checking mass: {} and result {} ... '.format(mass, result), end='')
    assert result == calc_fuel_required(mass)
    print('OK')
...
result_fuel_required = []
for mass in get_data():
    result_fuel_required.append(calc_fuel_required(mass))

print('Result:', sum(result_fuel_required))
...
module_fuel_required = []
fuel_fuel_required = []


def calc_fuel_fuel_required(module_fuel):
    mass = calc_fuel_required(module_fuel)
    if mass > 0:
        mass += calc_fuel_fuel_required(mass)
    else:
        mass = 0
    return mass
...
def calc_part2(data):
    ...
    for mass in data:
        fuel = calc_fuel_required(mass)
        fuel_fuel = calc_fuel_fuel_required(fuel)
        # print('>>', fuel, fuel_fuel)
        module_fuel_required.append(fuel)
        fuel_fuel_required.append(fuel_fuel)

    return sum(module_fuel_required + fuel_fuel_required)
...
for data_input, expected in test_set:
    result = calc_part2(data_input)
...
print('-' * 30, 'Result')
print('Result:', calc_part2(get_data()))
