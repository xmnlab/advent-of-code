...
def split_opcodes(opcodes: str):
    result = []
    i = 0
    
    opcodes_list = [_int(v) for v in opcodes.split(',')]

    while i < len(opcodes_list):
        if opcodes_list[i] == 99:
            result.append(opcodes_list[i:i+1])
            i += 1
        else:
            result.append(opcodes_list[i:i+4])
            i += 4
    return result
...
def calc_intcodes(opcodes_list: list):
    result = opcodes_list[:]
    i = 0
    while i < len(result):
        v = result[i]
...
# test 1
opcodes_0 = '1,9,10,3,2,3,11,0,99,30,40,50'
opcodes_lines_0 = [
    [1,9,10,3],
    [2,3,11,0],
    [99],
...
test_set = (
    (opcodes_0, opcodes_lines_0),
)

for opcodes, opcodes_lines in test_set:
    r = split_opcodes(opcodes)
    print('Expected:', opcodes_lines)
    print('Result:', r)
    assert r == opcodes_lines
...
print('Result:', result)
print('What value is left at position 0 after the program halts?', 'R:', result[0])
